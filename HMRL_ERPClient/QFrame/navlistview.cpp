	...
	if (node->children.size() != 0) {
		...
		if (nav->getIcoStyle() == NavListView::IcoStyle_Cross) {
			...
			if (node->collapse) {
                ...
                points.append(QPointF(6, 16));
                points.append(QPointF(16, 10));
			} else {
				points.append(QPointF(6, 4));
				points.append(QPointF(18, 4));
                points.append(QPointF(12, 12));
			}
	...
	for (int i = 0; i < count; i++) {
		...
		if (fatherTitle.isEmpty()) {
			...
			for (int j = 0; j < count; j++) {
                ...
                QString secCollapse = secList.at(2);
				...
				if (secFatherTitle == title) {
                    ...
                    secNode->collapse = secCollapse.toInt();
					secNode->level = 2;
...
NavListView::NavListView(QWidget *parent) : QListView(parent)
{
    ...
    style = NavListView::IcoStyle_Triangle;

	colorLine = QColor(214, 216, 224);

    colorBgNormal = QColor(200, 204, 240);
    colorBgSelected = QColor(133, 195, 124);
