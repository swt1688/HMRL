...
quint16 VerifyChecksum::crc16ForModbus(const QByteArray &data)
{
...
unsigned int VerifyChecksum::CRC16(const QByteArray &puchMsg)//, unsigned int usDataLen
{
    unsigned int reg16;
    ...
    int i,j;
    reg16=0xFFFF;
    for(i=0;i<puchMsg.size();i++)
    {
        regHi=(reg16>>8)&0x00FF;
        regLow=reg16&0x00FF;
        charCheck=puchMsg.at(i);
...
QByteArray VerifyChecksum::gemfieldCRC(QByteArray gemfield)
{
    QByteArray temp;
...
quint16 VerifyChecksum::crc16ForX25(const QByteArray &data)
{
    return qChecksum( data.data(), data.size() );
}

quint32 VerifyChecksum::crc32(const QByteArray &data)
{
